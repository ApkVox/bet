...
function setTheme(theme) {
    ...
    const icon = theme === 'dark' ? 'ÔÿÇ´©Å' : '­ƒîÖ';
...
function showToast(message, type = 'info') {
    ...
    toast.innerHTML = `
        <span>${type === 'success' ? 'Ô£à' : type === 'error' ? 'ÔÜá´©Å' : 'Ôä╣´©Å'}</span>
        <span>${message}</span>
    `;
...
async function loadPredictions() {
    ...
    try {
        ...
        if (data.status === "pending_github_actions") {
            const icon = currentSport === 'football' ? 'ÔÜ¢' : '­ƒÅÇ';
            grid.innerHTML = `
                <div class="empty-state" style="padding: 40px 20px;">
                    <div class="empty-icon" style="font-size: 48px; margin-bottom: 16px;">ÔÅ│</div>
                    <h3 style="margin-bottom: 8px;">Calculando Predicciones</h3>
                    <p style="color: var(--text-secondary); max-width: 400px; margin: 0 auto; line-height: 1.5;">El motor de Inteligencia Artificial en GitHub Actions est├í procesando los partidos de hoy. Por favor regresa en unos minutos.</p>
                </div>`;
            return;
        }
        ...
        const icon = currentSport === 'football' ? 'ÔÜ¢' : '­ƒÅÇ';
        grid.innerHTML = `
            <div class="empty-state"><div class="empty-icon">${icon}</div><p>Error de conexi├│n con el servidor de lectura</p></div>`;
...
async function loadHistory() {
    ...
    try {
        ...
        container.innerHTML = `<div class="empty-state"><div class="empty-icon">ÔÜá´©Å</div><p>Error cargando historial</p></div>`;
        ...
        if (currentSport === 'football') {
            ...
            <div class="prediction-card" onclick="showDetails(${idx})">
                ...
                <div class="prediction-meta">
                    ...
                    ${pred.warning_level === 'HIGH' ? `<span class="meta-tag negative">ÔÜá´©Å Riesgo</span>` : ''}
                </div>
            </div>
            `;
        }
...
function renderHistory(filter) {
    ...
    if (!filtered.length) {
        container.innerHTML = `<div class="empty-state"><div class="empty-icon">­ƒô¡</div><p>No hay registros</p></div>`;
        return;
    }
                            ...
                            <div class="history-card">
                                ...
                                <div class="history-info">
                                    ...
                                    <span class="result-badge ${h.result?.toLowerCase() || 'pending'}">
                                        ${h.result === 'WIN' ? 'Ô£ô Ganada' : h.result === 'LOSS' ? 'Ô£ù Perdida' : 'ÔÅ│ Pendiente'}
                                    </span>
                                </div>
                            </div>
                        `;
    ...
    if (filtered.length > currentHistoryLimit) {
        html += `
            <div class="load-more-container">
                <button class="load-more-btn" onclick="loadMoreHistory()">Ver m├ís partidos...</button>
            </div>
        `;
    }
...
function showDetails(index) {
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <p class="analysis-text">${pred.ai_analysis || 'An├ílisis de IA no disponible para este partido.'}</p>
    `;
